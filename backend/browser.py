...
class GetVariants(handlers.UnsafeHandler):
    def get(self, dataset, datatype, item):
        ret = {'variants':[], 'headers':[['variantId','Variant'], ['chrom','Chrom'], ['pos','Position'],
                                         ['HGVS','Consequence'], ['filter','Filter'], ['majorConsequence','Annotation'],
                                         ['flags','Flags'], ['alleleCount','Allele Count'], ['alleleNum','Allele Number'],
                                         ['homCount','Number of Homozygous Alleles'], ['alleleFreq','Allele Frequency']]}
        try:
            db = connect_db(dataset, False)
            db_shared = connect_db(dataset, True)
            try:
