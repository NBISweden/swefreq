...
docker run -v swefreq-mysql-data:/var/lib/mysql --rm --name mysql -e MYSQL_ROOT_PASSWORD=$MYSQL_PASS -d mysql:5.7 >/dev/null


# Wait for it to start
while ! docker run -i --rm --link mysql:mysql mysql:5.7 mysql -h mysql -uroot -p$MYSQL_PASS -e exit 2>/dev/null; do
...
docker run -i --rm --link mysql:mysql mysql:5.7 mysql -h mysql -uroot -p$MYSQL_PASS <<__END__
